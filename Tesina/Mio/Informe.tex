...
modelo mixto sin consideraciones adicionales. Esto se debe a que no habrá problemas
con la estimación de los parámetros, ya que se cumple el supuesto de
...
$ Y_{ij} = \beta_0 + \beta_1 \bar{X}_i + \epsilon_{i} $
...
$ Y_{ij} = \beta_0 + \beta_1 X_{ij} t_j + \epsilon_{i} $

Si bien la incorporación de la covariable al modelo es válida, la interpretación
de su coeficiente al ser incluida de esta manera es un poco confusa.
...
rezagadas es resumir a través de una función la información de éstas en una
sola covariable. Un ejemplo puede ser el valor promedio o acumulado hasta la
ocasión actual. Sin embargo, la elección de está funcion dependerá del tipo de
problema a analizar. Cabe destacar que, al igual que con toda medida resumen, al
usar este tipo de covariables se pierde parte de la información.
