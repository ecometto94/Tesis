...
X_{ik}$ para todo $j, k$. Con las CVT estocásticas, que se
...
\[ Y_{ij} = \beta_0 + b_{0i} + \beta_1\ \overline{X}_i + \beta_2\ t_j + \beta_3\ t_j\ \overline{X}_i + b_{1i}\ t_j + \varepsilon_{ij} \]

El coeficiente $\beta_1$ se interpreta como el cambio promedio en la ordenada al
origen y el coeficiente $\beta_3$ es el cambio promedio en la pendiente de la
tension arterial, por cada cambio unitario en el IMC promedio del individuo.
...
\[ Y_{ij} = \beta_0 + b_{0i} + \beta_1\ X_{ij} + \beta_2\ t_j + \beta_3\ t_j\ X_{ij} + b_{1i}\ t_j + \varepsilon_{ij} \]

Como se mencionó anteriormente, las CVT contienen efecto intra-unidad y
entre-unidad, sin embargo el modelo lineal mixto va a tener solo una covariable
$X_{ij}$. Si bien el modelo es válido, para obtener una interpretación clara del
efecto de la CVT se necesita dividir su efecto en dos coeficientes, como se
...
\[ Y_{ij} = \beta_0 + b_{0i} + \beta_{1k}\ X_{ij-k} + \beta_2\ t_j + \beta_3\ t_j\ X_{ij-k} + b_{1i}\ t_j + \varepsilon_{ij} \]
...
Una alternativa cuando se quiere utilizar toda la información de las covariables
...
\[ Y_{ij} = \beta_0 + b_{0i} + \beta_1\ \overline{X}_{ij} + \beta_2\ t_j + \beta_3\ t_j\ \overline{X}_{ij} + b_{1i}\ t_j + \varepsilon_{ij} \]
...
\[ 
	Y_{ij} = \beta_0 + b_{0i} + \beta_{0W}\ (X_{ij} - \overline{X}_i) + \beta_{0B}\ \overline{X}_i
	+ \beta_2\ t_j + \beta_{1W}\ t_j\ (X_{ij} - \overline{X}_i) + \beta_{1B}\ t_j\ \overline{X}_i + b_{1i}\ t_j + \varepsilon_{ij}
\]

Donde, $\overline{X}_i$ representa el promedio de todos los valores observados
en el tiempo de la CVT para la unidad $i$. $\beta_{0W}$ representa el cambio
esperado en la media de la variable respuesta asociado con cambios de la CVT
dentro de la unidad y $\beta_{0B}$ representa el cambio esperado en la media de
la variable respuesta asociado con cambios de la CVT entre unidades, ambos
afectando a la ordenada al origen. Mientras que $\beta_{1W}$ y $\beta_{1B}$
representan los mismos cambios asociados a la pendiente.

En el ejemplo del IMC y la tensión arterial, $\beta_{W0}$ se interpreta como el
cambio promedio en la tensión arterial el tiempo 0 por cada diferencia de una
unidad en el IMC de la ocasión $j$ comparado con el IMC promedio de la persona,
manteniendo constante el IMC de las ocasiones restantes. Por otro lado,
$\beta_{B0}$ se interpreta como el cambio promedio en la tensión arterial en el
tiempo 0 por cada cambio de una unidad en el IMC promedio de la unidad,
controlando por el valor del IMC de la ocasión $j$. De la misma manera
$\beta_{W1}$ y $\beta_{B1}$ representan los cambios promedio en las pendientes
de los individuos a través del tiempo.

Estos efectos permiten medir entonces si la tensión arterial aumenta cuando una
persona tiene un IMC mayor que el promedio de la población, y también permiten
medir el cambio en la tensión arterial si dicha persona tiene IMC mayor que su
promedio habitual.
...
facilmente interpretable. Sin embargo, los coeficientes de la tabla indican que,
manteniendo constante el efecto del resto de las covariables, \hl{los pacientes que
adhieren al tratamiento presentan una disminución promedio en su TAS de 1,524
unidades al primer mes del tratamiento, acompañado de una disminución de 0,792 unidades por
mes menores a los pacientes que no adhieren. Es importante aclarar que, al ser
la adherencia una CVT, esto no es una pendiente. Es decir, si un paciente
adhiere al segundo mes, su TAS promedio sera 2,364 unidades menor
($-1,524*1 + 0,371*2 - 0,791*2*1$) mientras que un paciente que no adhiere tendra
una TAS promedio 0,742 unidades mayor ($-1,524*0 + 0,371*2 - 0,791*2*0$).
Sin embargo, si el mismo paciente adhiere al tercer mes, su TAS será en promedio
2,784 unidades menor ($-1,524*1 + 0,371*3 - 0,791*3*1$) que un paciente que no
adhiere.}
...
variables, \hl{los pacientes que adhieren en la totalidad del tratamiento,
presentan una TAS promedio menor en 2,763 unidades el primer mes acompañado de
un descenso de 0,272 unidades por mes.}
...
disminución promedio de 2,838 en la TAS en relación a la proporción de
adherencia total al primer mes de tratamiento y además disminuirá en 1,010 de
manera proporcional a la proporción adherencia total por mes. Por ejemplo, un
paciente que adhiere sólo al 50\% del tratamiento, tendrá una disminución
promedio en su TAS de 1.419 ($2,838\ *\ 0.5$) en el primer mes del tratamiento
acompañado de una disminución de 0,505 por mes ($1,010\ *\ 0.5$).}
...
covariables, \hl{los pacientes que adhieren en el mes anterior al tratamiento
presentan un aumento promedio en su TAS de 0,189 unidades al primer mes del
tratamiento, acompañado de una disminución de 0,443 unidades por mes menores a
los pacientes que no adhieren.}
...
ocasiones hasta la actual presentan una disminución promedio en su TAS de 3,247
unidades al primer mes del tratamiento, acompañado de una disminución de 0,835
unidades por mes menores a los pacientes que no adhieren en ninguna ocasión
($\overline{adherencia}_{ij}$ = 0).}
...
coeficientes -2.055 y -0,603 quieren decir que, manteniendo constante la
proporción de adherencia total, se espera que los pacientes que adhieren al
tratamiento presenten una disminución promedio en la TAS de 2.055 unidades al
primer mes del tratamiento y que esta disminuya en promedio 0,603 por cada mes
en el que se adhiere al tratamiento. Por otro lado, los coeficientes 0,662 y
-0,525 quieren decir que, después de controlar por la adherencia en ese mes, se
espera que la TAS el primer mes de tratamiento sea mayor en 0.662 unidades en relación
a la proporción de adherencia total y luego disminuya, en promedio, 0,525
unidades por mes en proporcion a la adherencia total al tratamiento.}
...
BIC, los modelos que mejor ajustan los datos son el modelo 1 \ref{modelo_1} y el
modelo 6 \ref{modelo_6}, es decir, los modelos que incorporan la CVT en su
manera natural sin aplicarse ninguna transformación. El modelo 6 presenta
valores un poco superiores, esto se debe a que se agregan coeficientes que no
son significativos pero logran facilitar su interpretación.
