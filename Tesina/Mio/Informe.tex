...
\begin{itemize}
	...
	\item \textit{Variación biológica intra-unidad:} Se piensa que existe algún
	proceso que genera las respuestas de una unidad, el cual cambia en forma
	continua y suave a través del tiempo produciendo que las medidas más
	cercanas sean más parecidas.
...
%%%% CONSULTAR POR LA ULTIMA ORACION, CUALQUIER OTRA ESTRUCTURA?
...
Pepe y Anderson (1994) mostraron que las ecuaciones anteriores llegan a cero
solo si los datos cumplen con el supuesto:
...
elección de cuantos predictores rezagados utilizar y sobre la estructura de sus
coeficientes.
...
sólo mantienen efectos por una corta duración. En este caso, si la covariable es
...
el supuesto de independencia condicional y pueden recibir una interpretación
causal.
...
presentó dicha covariable, por lo que el método anterior resultará en valores
extraños para $\beta_W$. Por ejemplo, si la CVT se presentó en el 50\% de las
ocasiones, $\overline{X}_i$ tendrá un valor de 0.5 y entonces el término que
...
estará acompañado de un 0). Una forma de evitar esto es dividiendo el efecto
